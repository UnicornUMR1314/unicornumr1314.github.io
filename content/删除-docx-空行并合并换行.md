Title: 删除 Word (.docx) 中的空行并合并连续换行 — dele_null_line.py 使用说明
Category: 工具
Date: 2025-11-13 10:00
Tags: python, docx, 文档处理
Slug: delete-docx-empty-lines-merge-newlines
Summary: 一个用于清理 Word (.docx) 文档中空段落并合并连续换行的轻量级 Python 工具，带 GUI 交互。

本文介绍仓库内的脚本 `原始内容文档/dele_null_line.py`，它可以自动删除 `.docx` 文档中的空段落，并在非空段落中合并连续换行（将多个连续换行替换为一个）。脚本同时会处理表格单元格内的段落。
